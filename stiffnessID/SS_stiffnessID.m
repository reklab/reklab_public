function [system_ss,pIntrinsic,  tqPredicted]=  SS_stiffnessID (z,varargin)
...
%Identification of Human Ankle Joint Stiffness", IEEE Transactions on
%Biomedical Engineering, vol. 58, no. 11, pp.3039-3048, 2011.
%
options={{'decimation_ratio' 10 'ratio to decimate data'} ...
         {'irf_len' 2.5 'length of linear component IRF in s'} ...
         {'order' 8 'maximum order for nonlinearity'} ...
         {'hankle_size' 20 'Size of hankle matrix'} ...
         {'reflexdelay' 0.04 'Delay added to the input'} ...
     };
 if arg_parse(options,varargin);
     return
 end
%Written based on PhD work of Yong Zhao
%%
incr=get(z,'domainIncr');
POS=z(:,1);
VEL=ddt(POS);
DVEL=delay(VEL, ceil(reflexdelay/incr));
ACCEL=ddt(VEL);
TQ=z(:,2);

%% decimate
pos=double(decimate(POS,decimation_ratio));
vel=double(decimate(VEL,decimation_ratio));
dVel=double(decimate(DVEL,decimation_ratio));
accel=double(decimate(ACCEL,decimation_ratio));
tq=double(decimate(TQ,decimation_ratio));
outputIncr=incr*decimation_ratio;
% Generate costructed inputs

cInputIntrinsic = [pos vel accel]; % Constructed input for inrinsic torques
cInputIntrinsic= detrend(cInputIntrinsic);
cInputReflex = basisinput(dVel,order); % Constructed  input for reflex torques (Cheb outputs)

inputTotal = [ cInputIntrinsic cInputReflex]; % Overall construscted input
inputReflex = [zeros(size(cInputIntrinsic)) detrend(cInputReflex)]; % Input for generating estimated reflex torques 
inputIntrinsic = [cInputIntrinsic zeros(size(cInputReflex))]; % Input for generating esimated intrinisc torques
 

[S, R] = dordpi(inputTotal,tq,hankle_size); % Subsampe model with only information from internal states
system_order = orderselect(S);
[AT CT] = destac(R,system_order);
abs(eig(AT));
f=find( abs(eig(AT))>1, 1);
if ~isempty(f)
    error('The identified system is unstable.');
end
if system_order>0
    [BT,DT] =  destbd_ankle(inputTotal,tq,AT,CT);
    K=DT(1);
    B=DT(2);
    I=DT(3);
    system_ss=ss(AT,BT,CT,DT,incr*decimation_ratio);
    tqT = dlsim(AT,BT,CT,DT,inputTotal); % Simulate net torque 
    tqI = dlsim(AT,BT,CT,DT,inputIntrinsic); % simulate intrinsic torque
    tqR = dlsim(AT,BT,CT,DT,inputReflex); % Simulate reflex torque 
    ...
    AT=[0];
    CT=[0];
    DT=[K B I zeros(1,order)];
    BT=zeros(size(DT));
    system_ss=ss(AT,BT,CT,DT,outputIncr);
    out=reg*theta;
    tqI=nldat(out,'DomainIncr',outputIncr);
    tqT=tqI;
    tqR=0*tqI;
  
end
     tqInput=nldat(tq,'domainIncr',outputIncr,'chanNames',{'Input torque'});
     tqT=nldat(tqT,'domainIncr',outputIncr,'chanNames',{'Total torque'});
     tqI=nldat(tqI,'domainIncr',outputIncr, 'chanNames',{'Intrinisc torque'});
     tqR=nldat(tqR,'domainIncr',outputIncr, 'chanNames',{'reflex torque'});
     tqPredicted= cat(2,tqInput, tqT, tqI, tqR); 
     set (tqPredicted, 'comment','Torques predicted using state space stiffness identification');
     pIntrinsic.K=K;
     pIntrinsic.B=B;
     pIntrinsic.I=I;
end
function [B,D,Phi,x0,K,R] =  destbd_ankle(u,y,A,C,model,Rold,iv,niv)
...
N = size(y,1); %data length 
l = size(y,2); %number of output
m = size(u,2); %number of input
n = size(A,1); 
if ~(size(u,1)==N) 
...
nparam = n+fB * n * (m-3)+fK * n * l+fD * m * l;% number of parameters 
...
eb = n+fB * (m-3) * n+fK * n * l+fD * m * l; % end of block from R 
...
if fB 
  B = zeros(n,(m-3));B(:) = XBD(fx * n+1:fx * n+n * (m-3)); 
  B = T * B; 
  B = [zeros(n,3) B];
end 
if fK 
...
if fD 
%   size(XBD)
  D = zeros(m,l);D(:) = XBD(fx * n+fB * n * (m-3)+fK * n * l+1:fx * n+fB * n * (m-3)+fK * n * l+l * m); 
  D = D'; 
end 
...
if fB 
  ...
  for i = 1:n 
    ...
    Cout = C1(:,1:s); 
    for j = 1:(m-3) 
      x  =  ltitr(Aout,Bout,u(:,(j+3)),zeros(size(Aout,1),1)); 
      ybij  =  x  *  Cout.' ; 
      Ybij(:,(j-1) * n+i) = ybij(:); 
    end 
  end 
