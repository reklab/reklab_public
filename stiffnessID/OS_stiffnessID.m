function [system_ss, K, B, I, tqR, tqI, tqT]=  SSY_stiffnessId (z,varargin)
...
options={{'decimation_ratio' 10 'ratio to decimate data'} ...
         ...
         {'delay' 0.04 'Delay added to the input'} ...
     };
...
POS=z(:,1);
VEl=ddt(POS);
DVEL=delay(VEL, ciel(delay/incr));
ACCEL=ddt(VEL);
TQ=z(:,2);
...
outputIncr=incr*decimationRatio
...
inputIntrinsic = [cInputIntrinsic zeros(size(u1))]; % Input for generating esimated intrinisc torques
inputReflex = [zeros(size(input)) detrend(cInputReflex)]; % Input for generating estimated reflex torques 
