...
pcIRF=elements{1};
Ts=pcIRF.domainIncr;
max_order =  get(pc_system,'polyOrderMax');
...
wskernel = wkern;
set(wskernel,'order',kernOrder,'Ts',Ts,'nLags',hlen);
...
polytype = get(m,'polyType');
if strcmp(polytype,'hermite')
  m = polynom(m,'polyType','tcheb');
end
set(m,'ployStd',sigma_x,'polyMean',0);
nh_poly = polynom(m,'polyType','hermite');
coeffs = get(nh_poly,'ployCoeffs');
order = get(nh_poly,'ployOrder');
for i = 1:order
  coeffs(i+1) = coeffs(i+1)/(sigma_x^i);
end
set(nh_poly,'polyCoeffs',coeffs);
end
