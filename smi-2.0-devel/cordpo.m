 function [Sn,Rnew] = cordpo(u,y,t,a,i,Rold); 
 % cordpo  Delivers information about the order of the LTI continuous time 
...
%         disturbance on the output, independent of the noise-free input u(t). 
%         This function uses the past output instrumental variable and is 
%         comparable to dordpo for the discrete time case. 
% 
% Syntax 
%         [Sn,Rnew]=cordpo(u,y,t,a,i); 
%         [Sn,Rnew]=cordpo(u,y,t,a,i,Rold); 
% 
% Input: 
...
%         with cordpo, containing the triangular factor, estimated 
...
Hheight = 2 * i+i * (2 * m+2 * l);Hwidth = Ns; 
 if i<1, 
...
L = triu(qr([L';[X0,Uf,Up,Yp,Yf]]))'; 
L = L(1:nf+(2 * m+2 * l) * i,1:nf+(2 * m+2 * l) * i); 
Lb = L(nf+(2 * m+l) * i+1:nf+(2 * m+2 * l) * i,nf+m * i+1:nf+(2 * m+l) * i); 
