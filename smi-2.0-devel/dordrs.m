 function [Sn,R] = dordrs(u,y,x,i,R); 
...
%         Data from different experiments can be concatenated using the 
%     extra input argument R. The estimated column-space is used in 
%     the function destac to extract the matrices A and C. This 
%         function implements the reconstructed state moesp algorithm 
...
if ~(size(x,1)==Ns) 
  error('The input and state must have same length.') 
end 
n = size(x,2); 
 
 
Hheight = i * (2 * m+l+n);Hwidth = Ns-2 * i+1; 
 if i<1, 
...
for k = (1:i), 
    ...
    Xe(:,(k-1) * n+1:k * n) = x(i+k:N+i+k-1,:); 
end 
L = triu(qr([L';[Uf Up Xe Yf]])); 
L = L(1:(2 * m+n+l) * i,1:(2 * m+n+l) * i)'; 
Lb = L((2 * m+n) * i+1:(2 * m+n+l) * i,m * i+1:(2 * m+n) * i); 
